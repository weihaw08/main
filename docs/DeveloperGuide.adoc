...
The following sequence diagram illustrates how a custom property is removed from all of the exercises
...
*Step 1:* The user first enters the name of the custom property which they wish to remove from the app.

*Step 2:* The parser will parse the command and after which, a `CustomRemoveCommand` object is created.

*Step 3:* The `execute` method of this `CustomRemoveCommand` object will be executed. Following which, the`CustomProperty`
object with the given name will be removed from `PropertyBook`.

*Step 4:* Once the removal of the `CustomProperty` object is done, the custom properties of every exercise in `Model` will
be updated.

*Step 5:* Finally, a `CommandResult` object is created and returned.
...
* provides custom properties which allow user to tailor suit the app to their own needs
